...
- Removed ~190 lines of commented-out proxy code from `cli/commands/serve.py`
- Documented `galleria/serializer/` as incomplete extraction infrastructure
- Verified all config files active, PIL usage correct, skipped tests are post-MVP placeholders

### Documentation Quality Assurance

- Created organize.md and benchmark.md for missing site commands
- Fixed broken links and config path references
- Updated galleria README status and nav links
- Added CONTRIBUTE.md and HANDOFF.md to doc/README.md
- Merged duplicate bconfiguration.md into configuration.md

### Release

- Version 0.1.0 tagged
- All tests passing (530 passed, 3 skipped)
...
- **COMPLETED: HTTP proxy handler implementation with comprehensive testing**
  ...
  - Added request forwarding logic for Galleria and Pelican servers with
    full HTTP support
...
- **COMPLETED: Comprehensive galleria test fixtures for serve command**
  ...
  - Specialized factories:
    - manifest_factory
    - gallery_output_factory (creates realistic HTML/CSS/thumbnails)
  - Complete scenarios:
    - complete_serving_scenario
    - file_watcher_scenario for end-to-end testing
  - HTTP testing: free_port, mock_http_server with request tracking
  - Updated E2E serve tests to use new fixtures
    - *(simplified from ~40 lines to ~4 lines setup)*
  - All 271 galleria tests passing
    - fixtures ready for comprehensive serve command testing
...
### Build Command Integration + Build Orchestrator Refactoring + Docs
...
- **COMPLETED: Pelican integration issues resolved**
  ...
  - Updated pelican.json schema with all required Pelican settings:
    - *(`THEME`, `IGNORE_FILES`, `DELETE_OUTPUT_DIRECTORY`, etc.)*
...
- **COMPLETED: Build orchestrator refactoring successfully finished**
  ...
  - Solution IMPLEMENTED:
    - Extracted business logic into dedicated orchestrator and builder classes
  - Results ACHIEVED:
  ...
  - Implementation: 7-commit incremental refactoring using strict TDD approach
    ...
    - Commit 4: PelicanBuilder
      - Pelican generation extraction (3 tests, 100% coverage)  
    ...
    - Commit 8: Unit test fixes
      - Updated all 9 build unit tests for orchestrator pattern
        - *(0 skipped, 9 passing)*

- **Commit 1 COMPLETED: Build module structure and exceptions**
  - Created build/ package for orchestration functionality
  - Added BuildError base exception with:
    - ConfigError
    - GalleriaError
    - PelicanError subclasses
...
- **RESOLVED: Build command broken after galleria integration attempt**
  - Galleria integration works: successfully uses
    - PipelineManager
    - plugin registration
    - direct imports
...
- **Complete JSON schema validation for all config types**
  ...
  - Create config/schema/pelican.json with required fields:
    - theme
    - site_url
    - author
    - sitename
  - Create config/schema/galleria.json designed for extraction-ready galleria package
  - Include optional fields like timezone, default_lang for pelican and
    thumbnail settings for galleria
...
- **Enhanced ConfigValidator with schema validation**
  ...
  - Load schemas from config/schema/ directory and
    validate config content against them
...
- **Build command integration with unified config system**
  ...
  - Create temporary galleria config files for CLI integration while
    maintaining extraction-ready design
...
- **Complete E2E test validation and system integration**
  ...
  - Update galleria config format from pipeline structure to
    manifest_path/output_dir format
  - All 4 E2E config integration tests now pass:
    - valid configs
    - schema errors
    - missing files
    - JSON corruption
...
- **Complete build command TDD implementation**
  ...
  - Add 8 unit tests covering:
    - organize cascade
    - galleria/pelican integration
    - error handling
  - Implement cli/commands/build.py following organize command patterns
  - Use direct Python module imports
    -*(galleria.generate(), pelican.Pelican())* **not** CLI subprocess
...
- **Comprehensive organize E2E testing**
  ...
  - Verify symlink creation with
    actual NormPic filename patterns containing collection name
