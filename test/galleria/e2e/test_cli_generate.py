...
class TestGalleriaCLIGenerate:
    ...
    def test_cli_generate_handles_missing_config_file(self, tmp_path):
        ...
        assert any(
            ...
            for phrase in ["config", "not found", "no such file", "file not found"]
    ...
    def test_cli_generate_shows_help_with_no_args(self, tmp_path):
        ...
        assert any(
            ...
            for word in ["usage", "help", "config", "required", "missing option"]
    ...
    def test_cli_generate_validates_invalid_config_format(self, tmp_path):
        ...
        assert any(
            phrase in error_output
            for phrase in ["json", "invalid", "parse", "decode", "syntax"]
        ), f"Error should mention JSON parsing issue: {result.stderr}"

    def test_cli_generate_handles_malformed_content_gracefully(
        self, complete_serving_scenario
    ):
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            cwd=scenario["config_path"].parent.parent.parent.parent,
            timeout=30,
        )
    ...
    def test_cli_generate_timeout_protection_large_collections(
        self, manifest_factory, galleria_config_factory
    ):
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            cwd=config_path.parent.parent.parent.parent,
            timeout=60,
        )
        ...
        assert result.returncode is not None, (
            "Command should complete (not infinite loop)"
        )
