...
class TestSiteServeE2E:
    ...
    def test_site_serve_routing(
        ...
        # Arrange: Create site config and required files with absolute paths
        config_file_factory("site")
        config_file_factory("normpic")  # Required for build validation
        config_file_factory(
            "galleria",
            {
                "provider": {"plugin": "normpic-provider"},
        ...
        # Create Pelican content directory and source files
        content_dir = temp_filesystem / "content"
        content_dir.mkdir(parents=True)

        # Create index.md - Pelican will generate index.html from this
        (content_dir / "index.md").write_text("""Title: Home
Date: 2023-01-01
Slug: index

# Site Homepage from Pelican
This is the home page content generated by Pelican.
""")

        # Create about.md - Pelican will generate about.html from this
        (content_dir / "about.md").write_text("""Title: About
Date: 2023-01-01

# About Page
This is the about page content generated by Pelican.
""")
        ...
        # Build just Pelican content (avoid slow Galleria generation)
        build_result = subprocess.run(
            ["uv", "run", "pelican", "content", "-o", "output"],
            cwd=temp_filesystem,
            capture_output=True,
            text=True,
            timeout=10
        ...
        # Verify Pelican content was generated
        output_dir = temp_filesystem / "output"
        assert (output_dir / "index.html").exists(), f"Pelican should generate index.html. Build output: {build_result.stdout}\nError: {build_result.stderr}"
        assert (output_dir / "about.html").exists(), f"Pelican should generate about.html. Build output: {build_result.stdout}\nError: {build_result.stderr}"

        # Check that the generated content contains our expected text
        index_content = (output_dir / "index.html").read_text()
        about_content = (output_dir / "about.html").read_text()
        assert "Site Homepage from Pelican" in index_content, "Index should contain our content"
        assert "About Page" in about_content, "About should contain our content"
        ...
        try:
            ...
            # Pelican dev server shows directory listing when no index route is configured
            # This is expected behavior - the important thing is that about.html is listed
            assert b"about.html" in response.content, "Directory listing should show about.html"
            assert b"Directory listing" in response.content, "Should show Pelican directory listing"
